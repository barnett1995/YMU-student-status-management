...
public partial class t : System.Web.UI.Page
{
    ...
    public void show()
    {
        ...
        else
        {
            ...
            sushe.Value = dt.Rows[0]["sushe"].ToString();                           //宿舍
            zhiwu.Value = dt.Rows[0]["zhiwu"].ToString();                          //职务
    ...
    /// 可修改的html控制项
    /// </summary>
    public void htmlControls1()
    {
        shenqing.Visible = false;       //隐藏 提交申请button
        tijiao.Visible = true;            //显示 保存button 
    }

    /// <summary>
    /// 申请修改的html控制项
    /// </summary>
    public void htmlControls2(ControlCollection controls)
    {
        shenqing.Visible = true;          //显示 提交申请button
        tijiao.Visible = false;            //隐藏 保存button 

        foreach (Control item in controls)                          //遍历所有html控件
            {
            if (item.GetType() == typeof(HtmlInputText))
            {
                HtmlInputControl htmlControl = item as HtmlInputControl;
                htmlControl.Disabled = true;
            }
            else if (item.GetType() == typeof(HtmlInputFile))
            {
                HtmlInputFile htmlContorl1 = item as HtmlInputFile;
                htmlContorl1.Disabled = true;
            }
            else if (item.GetType() == typeof(HtmlTextArea))
            {
                HtmlTextArea htmlContorl2 = item as HtmlTextArea;
                htmlContorl2.Disabled = true;
            }
...

    {
        ...
        String su = sushe.Value;                                //宿舍号

        String zhi = zhiwu.Value;                               //职务
                     ...
                     // string savePath1 = Server.MapPath("C:\\web\\WebSite2\\picture\\");
                       ...
                       // string photoupStr = "UPDATE si SET Photo='" + wpath + "' WHERE ID='" + id + "'";
                        ...
                        else
                        {
                            ...
                            {
                                string updateStr = "UPDATE si SET Name='" + name + "',OnceName='" + lname + "',Marriage='" + hunyin + "',Nationality='" + minzu + "',Political='" + zhengzhi + "',Body='" + jiankang + "',Sex='" + xingbie + "',Birthday='" + shenri + "',E_Mail='" + youxiang + "',MobileNo手机号码='" + shouji + "', JTDH='" + jiazhang + "',SYD='" + chushengdi + "',CardID='" + shenfengzheng + "',JTZZ='" + huji + "',JZBM='" + youzheng + "',JoinTssue='" + rudang + "',Reward='" + jiangli + "',punishment='" + chengfa + "',BigEvents='" + wenti + "',OtherProblem='" + zuzhi + "',Remember='" + jishi + "',Bank='" + yinhang + "',Bankcard='" + kahao + "',Photo='" + wpath + "',Authority='false'  WHERE ID='" + id + "'";            //更新si表
    ...
    {
        string id = Session["ID"].ToString();
        string upsetStr = "UPDATE si SET apply='true' WHERE ID='"+id+"'";
        SqlConnection conn = new SqlConnection(connStr);
        SqlDataAdapter upda = new SqlDataAdapter(upsetStr, conn);
        DataSet upds = new DataSet();
        conn.Open();
        upda.Fill(upds);
        conn.Close();
        Response.Write(@"<script>alert('修改申请已提交成功，请耐心等待老师审核，不定期登录该网站查看审核结果');</script>");
    }
