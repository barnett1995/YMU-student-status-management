   ...
   string connStr = "server=1-PC;uid=sa;pwd=admin2017GAO;database=StudentsInfo";     //连接数据库
    ...
    protected void shangchuan_Click(object sender, EventArgs e)
    {
        if (FileUpload1.HasFiles)
        {
            ...
            if (!String.IsNullOrEmpty(newPath))                        //指示指定的字符串是 null 还是 Empty 字符串。
            {
                ...
                Response.Redirect("College4.aspx");
    ...
    private void DataSetOperator(DataSet ds, string filePath)
    {
        ...
        if (ds.Tables[0].Rows.Count < 1)                                              //判断是表内是否为空
        {
            ...
            this.ClientScript.RegisterStartupScript(this.GetType(), "alter", "alert('没有数据！')", true);
            return;
        }
        try          //检测异常语句
        {
        ...
        catch (Exception ex)                                                //获取一场信息
        {
            ...
            this.ClientScript.RegisterStartupScript(this.GetType(), "alter", "alert('发生异常,数据已回滚/n信息/n" + ex.Message + "')", true);
            str.Rollback();
        }
        finally                                                             //finally 块释放资源，例如，关闭在 try 块中打开的任何流或文件。
        {
            ...
            this.ClientScript.RegisterStartupScript(this.GetType(), "alter", "alert('上传成功" + k + " 条')", true);
    ...
    protected void add_ServerClick(object sender, EventArgs e)
    {
        ...
        if (id == "" || nm == "")
        {
            ...
            this.ClientScript.RegisterStartupScript(this.GetType(), "alter", "alert('增添的信息不能为空！')", true);
        }
        else
        {
            ...
            if (dt.Rows.Count == 1)                                               //从登录表读取数据，如果已经有输入信息，页面提示
            {
                ...
                this.ClientScript.RegisterStartupScript(this.GetType(), "alter", "alert('已有该条记录！')", true);
            }
            else
            {
                ...
                this.ClientScript.RegisterStartupScript(this.GetType(), "alter", "alert('上传成功！')", true);
    ...
    protected void chaxun_ServerClick(object sender, EventArgs e)
    {
        ...
        Application["nj"] = nj;
        Application["id"] = id;
        this.ClientScript.RegisterStartupScript(this.GetType(), "", "window.open('Gridview1.aspx','','height=800,width=1000,scrollbars=yes,status =yes')", true);

        // Response.Write("<script language='javascript'>window.open('Gridview1.aspx','','height=800,width=1000,scrollbars=yes,status =yes')</script>");
